import { Env, model_url, loader_configs, model, plant_configs, grid2pos, clamp, hydration_max } from "./common";
...
export class Configs {
    ...
    loadModelsPromise(): Promise<model> {
        ...
        return new Promise(resolve => {
            Promise.all(promises).then(
                models => resolve(Object.assign({}, ...models.map(s => ({ [s.name]: s }))))
            );
        })
    ...
    setWaterLevel(gridX: number, gridY: number, waterlevel: number) {
        this.env.grids[gridX][gridY].waterlevel = clamp(waterlevel, 0, hydration_max);
    }

    water(gridX: number, gridY: number): number {
        this.setWaterLevel(gridX, gridY, this.env.grids[gridX][gridY].waterlevel + 1);
        return this.env.grids[gridX][gridY].waterlevel;
    }

    addPlantModel(planttype: string, plantstatus: number, gridX: number, gridY: number): Object3D | undefined {
