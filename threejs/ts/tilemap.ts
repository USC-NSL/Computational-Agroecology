...
import { tilemap_color_configs, grid2pos, pos2grid, hydration_configs } from "./common";
...
export class TileMap {
    ...
    configs: Configs;
    render: Render;
    constructor(configs: Configs, render: Render, updateTile: (gridX: number, gridY: number) => void) {
    ...
    onDocumentMouseMove(event: { clientX: number; clientY: number; }) {
        ...
        if (this.mask != undefined) {
            ...
            else for (let material of <MeshBasicMaterial[]>this.mask.material)
    ...
    onDocumentMouseDown(event: { clientX: number; clientY: number; }) {
