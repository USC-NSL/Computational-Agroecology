...
export class MainControl {
    ...
    // modules
    configs: Configs; api: API; gui: GUI; tilemap: TileMap; render: Render;
    ...
    constructor(width: number, height: number) {
        ...
        this.tilemap = new TileMap(this.configs, this.render, this.updateTile.bind(this));
    ...
    updateTile(gridX: number, gridY: number, planttype: string = this.gui.getMode(), plantstatus: number = 0) {
        let plant: Object3D | undefined;
        switch (planttype) {
            case Mode[Mode.CORN]:
            case Mode[Mode.BEAN]:
            case Mode[Mode.SQUASH]:
                plant = this.configs.addPlantModel(planttype, plantstatus, gridX, gridY);
                if (plant !== undefined)
