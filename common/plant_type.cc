...
void PlantType::harvestProduce()
{
	...
	this->produceWeight = 0.0;
	for(unsigned int i = 0; i < harvestedProduce.size(); i++)
	{
		this->produceWeight+=harvestedProduce[i]->weight;
	}
	return;
};
// set default function for produce, that sets ripe to false, poisoned to false,
void PlantType::addProduce(bool ripeP, bool poisonedP, double weightP, double yieldFactor)
{
	if(this->name_ == "corn"){
		CornProduce* temp = new CornProduce(ripeP, poisonedP, weightP, yieldFactor);
...
void PlantType::updateRipeStatus(bool status, int numberChanged)
{
	...
	for(unsigned int i = 0; i < produceOnPlant.size(); i++)
	{
		if(count == 0) return;
