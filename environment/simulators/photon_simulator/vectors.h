 ...
 */
inline real_t squared_length(const Vector2& v) { return v.x * v.x + v.y * v.y; }

/**
 * Returns the length of a vector.
 */
inline real_t length(const Vector2& v) { return sqrt(squared_length(v)); }
 ...
 */
inline Vector2 vabs(const Vector2& v) { return Vector2(fabs(v.x), fabs(v.y)); }
...
inline Vector2 operator*(real_t s, const Vector2& rhs) { return rhs * s; }
 ...
 * Returns the dot product of two vectors
 */
inline real_t dotresult(const Vector3& lhs, const Vector3& rhs) {
  return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z;
}

// TODO: change back to original definition
 ...
 * Returns the length of a vector.
 */
inline real_t length(const Vector3& v) { return sqrt(squared_length(v)); }
...
inline Vector3 operator*(real_t s, const Vector3& rhs) { return rhs * s; }
...
inline Vector3 project(const Vector4& v) {
  real_t winv = v.w == 0.0f ? 1.0f : 1.0f / v.w;
 ...
 * Returns the length of a vector.
 */
inline real_t length(const Vector4& v) { return sqrt(squared_length(v)); }
 ...
 * Returns the unit vector pointing in the same direction as this vector.
 */
inline Vector4 normalize(const Vector4& v) { return v / length(v); }
...
inline Vector4 vmax(const Vector4& lhs, const Vector4& rhs) {
  return Vector4(std::max(lhs.x, rhs.x), std::max(lhs.y, rhs.y),
...
inline Vector4 vmin(const Vector4& lhs, const Vector4& rhs) {
  return Vector4(std::min(lhs.x, rhs.x), std::min(lhs.y, rhs.y),
                 std::min(lhs.z, rhs.z), std::min(lhs.w, rhs.w));
}

inline Vector4 operator*(real_t s, const Vector4& rhs) { return rhs * s; }
