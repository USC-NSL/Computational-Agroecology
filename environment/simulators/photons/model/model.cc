      ...
      // update normal index
      {
        ...
        if (invalid_normal_index && !smoothVertexNormals.empty()) {
          ...
          if (f0 >= 0 && f1 >= 0 && f2 >= 0) {
            ...
            normals_.push_back(smoothVertexNormals[f0]);
            v2.normal_index = normals_.size();
            normals_.push_back(smoothVertexNormals[f1]);
            v3.normal_index = normals_.size();
            normals_.push_back(smoothVertexNormals[f2]);
...
static void computeSmoothingNormals(
  ...
  for (size_t f = 0; f < shape.mesh.indices.size() / 3; f++) {
    ...
    for (size_t i = 0; i < 3; ++i) {
      iter = smoothVertexNormals.find(vertex_index[i]);
      if (iter != smoothVertexNormals.end()) {
        ...
        smoothVertexNormals[vertex_index[i]].x = normal[0];
        smoothVertexNormals[vertex_index[i]].y = normal[1];
        smoothVertexNormals[vertex_index[i]].z = normal[2];
      }
    }
