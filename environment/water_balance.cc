...
double WaterBalance::WaterContentBeforeRedistribution(double soil_layer_thickness, double water_content, double percolation_from_above)
{
   ...
   double water_content_saturation = saturation_point * soil_layer_thickness * kMeterToMillimeter; // for conversion to mm
...
{
   double soil_thickness_mm = soil_thickness * kMeterToMillimeter;  // for conversion
...
WaterBalance::ActualEvaporationReturn 
      WaterBalance::ActualEvaporation(double potential_evaporation, double water_amount_1)
{
      ...
      double photosynthesis_efficiency, double potential_transpiration, double water_amount_2)
{
   ...
   ActualTranspirationReturn acutal_transpiration_return = {
      0.0,
      potential_transpiration * reduction_factor
   };
   return acutal_transpiration_return;
}

WaterBalance::VolumetricWaterContentReturn WaterBalance::VolumetricWaterContent(
      double soil_thickness, double volumetric_water_content, double actual_evaporation, 
      double actual_transpiration, double percolation_from_above)
{
   double soil_thickness_mm = soil_thickness * kMeterToMillimeter;     // for conversions
   ...
   double percolation_to_below = water_content_before - actual_evaporation - actual_transpiration - water_content_after;  
   if (percolation_to_below < 0.0) {
      percolation_to_below = 0.0;
   }
   VolumetricWaterContentReturn volumetric_water_content_return = {
      water_content_after / soil_thickness_mm,
      percolation_to_below
   };
   return volumetric_water_content_return;
}

 // TODO: When calling this function,
 //    Get dETs and dETc from "meteorology.cc" file, from UpdateHourlyNetRadiation (or maybe some other function)
WaterBalance::DailyWaterContentReturn WaterBalance::DailyWaterContent(
      double rainfall, double water_amount_1, double water_amount_2, double dETs, double dETc)
{
   double potential_evaporation = dETs * portential_factor;
   double potential_transpiration = dETc * portential_factor;
   ActualEvaporationReturn actual_evaporation = ActualEvaporation(potential_evaporation, water_amount_1);
   ...
   VolumetricWaterContentReturn water_content_layer_1 = 
         VolumetricWaterContent(depth_1, water_amount_1, actual_evaporation.layer_1, 
               actual_transpiration.layer_1, rainfall);
   VolumetricWaterContentReturn water_content_layer_2 = 
         VolumetricWaterContent(depth_2, water_amount_2, actual_evaporation.layer_2, 
               actual_transpiration.layer_2, water_content_layer_1.percolation_to_below);
   DailyWaterContentReturn DailyWaterContent = {
      water_content_layer_1.volumetric_water_content,
      water_content_layer_2.volumetric_water_content
   };
   return DailyWaterContent;
}

// Fraction of growth reduced due to limited water. potT is potential
//   transpiration (mm day-1).
double WaterBalance::GrowthReduction(double potential_transpiration, double water_amount_2)
{
   ActualTranspirationReturn actual_transpiration = ActualTranspiration(
         photosynthesis_efficiency_c3, potential_transpiration, water_amount_2);
   return (actual_transpiration.layer_2 / potential_transpiration);       // fraction (0 to 1)
}
